...
publish)
  ...
  if [[ $REPLY =~ ^[Yy]$ ]]
    then
      ...
      # 本地最新tag和即将生成的tag相同 则删除
      if [ "$LatestTag" == "$VERSION" ];then
        git tag --delete $VERSION
      fi

      BRANCH="release-v$VERSION-$(date "+%F")"
      git checkout -b $BRANCH
      npm version $VERSION --message "docs: $VERSION"
      ...
      git tag -a "v$VERSION"
      # 先删除远程分支
      # git -C "${REPO}" push origin ":refs/tags/v$VERSION"
...
#  BRANCH="develop"
#  read -p"Target branch: master111. (y: confirm; n: enter the target branch; c: cancel) " -n 1 -r
#  echo "\n"
#
#  case $REPLY in
#     y)
...
#  esac
  ...
  #pbcopy <$OUTPUT
  # 完成
  echo "Push success, Release branch is $BRANCH and version is $VERSION !"
  ;;
copy)
  genTagLog "copy"
